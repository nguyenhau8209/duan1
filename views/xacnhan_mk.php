    ...
    <div class="flex flex-col lg:flex-row w-10/12 lg:w-8/12 bg-white mx-auto shadow-lg overflow-hidden">
      ...
      <div class="w-full lg:w-1/2 py-16 px-12">
        <div class="flex justify-around pb-3 cursor-pointer border-b-2  ">
          <h2 class="font-bold text-lg text-[#002D74] border-b-2 border-[#F10000]">Nhập mã xác nhận</h2>

        </div>

        <form action="?act=Confirm_password" method="POST">
          <p class="bg-[#70a7e5] rounded-md  text-white p-2">
            Hãy nhập mã xác nhận mà chúng tôi đã gửi về cho Email của bạn
          </p>
          <div class="mt-8">
            <input type="text" placeholder="Nhập Mã " name="Confirm" class="border border-gray-400 py-1 px-2  rounded-md h-[40px] w-full">
            <?php
            if (!empty($error['error']))
              echo "<p class='error pt-3'>{$error['error']}</p>"
            ?>
          </div>
